...
from std_msgs.msg import String, Bool, Empty, Float32MultiArray
...
class ApproachClient():
  ...
  def __face_callback(self, detections : FacePositions):
    ...
    if self.__depth_image is not None:
      ...
      if face_dist > (1+self.__tolerance)*self.__approach_dist and not np.isnan(face_dist):
        ...
        # self.__navigator.goto(map_point.point.x, map_point.point.y, np.pi-np.arctan(map_point.point.x/map_point.point.y)) # AC
        # AC: for megagenial only
        print(self.__navigator.get_state(), '==', GoalStatus.SUCCEEDED, 'or', GoalStatus.LOST)
        if self.__navigator.get_state() == GoalStatus.SUCCEEDED or self.__navigator.get_state() == GoalStatus.LOST:
          rospy.loginfo("approaching detected client")
          center_face_in_img = ToEulerAngles(map_pose.pose.orientation).z
          res = 'O' # input('Oui/Non?')
          if res == 'O':
            result = self.__navigator.goto(map_pose.pose.position.x, map_pose.pose.position.y, center_face_in_img)
  ...
  def __camera_callback(self, image : Float32MultiArray):
